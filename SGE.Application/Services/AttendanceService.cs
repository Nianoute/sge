...
{
    /// <summary>
    ///     Registers the clock-in time for an employee. If an attendancerecord for the employee on the specified day
    ///     already exists without a clock-in time, it updates the recordwith the provided clock-in information.
    ///     Otherwise, it creates a new attendance record.
    /// </summary>
    /// <param name="clockInDto">An object containing the employee's IDand clock-in time information.</param>
...
// Mettre à jour l'entrée existante
    ...
    /// <exception cref="KeyNotFoundException">Thrown when the employee corresponding to the specified ID is not found.</exception>
    /// <exception cref="InvalidOperationException">
    ///     Thrown when no clock-in record exists for the specified day, when the employee has not clocked in prior to clocking
    ///     out,
...
// Vérifier que l'employé existe
        if (!await
                employeeRepository.ExistsAsync(clockOutDto.EmployeeId,
                    cancellationToken))
...
// Mettre à jour les informations de la présence
    ...
    ///     An object containing the details needed to create the attendance record, including
    ...
    /// <exception cref="InvalidOperationException">
    ///     Thrown if there is an issue during the creation of the attendance record or
    ///     calculation of hours.
    /// </exception>
    public async Task<AttendanceDto>
        CreateAttendanceAsync(AttendanceCreateDto createAttendanceDto,
            CancellationToken cancellationToken = default)
    {
//TODO
        throw new NotImplementedException();
...
// SI existe : throw newInvalidOperationException($"Attendance record already exists for {createAttendanceDto.Date:yyyy-MM-dd}");

        var entity = mapper.Map<Attendance>(createAttendanceDto);
// Calculer les heures travaillées si les deux heures sontfournies
    ...
    public async Task<IEnumerable<AttendanceDto>>
        GetAttendancesByEmployeeAsync(int employeeId,
...
// TODO
    ...
    /// <param name="cancellationToken">A token to observe during the asynchronous operation for cancellation.</param>
    /// <returns>
    ///     An attendance DTO containing the details of the employee's attendance for the current date, or null if no
    ///     record exists.
    /// </returns>
    /// <exception cref="KeyNotFoundException">Thrown when no employee is found with the specified ID.</exception>
    /// <exception cref="InvalidOperationException">
    ///     Thrown when multiple attendance records for the employee are found for the
    ///     current date.
    /// </exception>
    public async Task<AttendanceDto?> GetTodayAttendanceAsync(int
            employeeId,
        CancellationToken cancellationToken = default)
    {
// TODO
    ...
    public async Task<decimal> GetMonthlyWorkedHoursAsync(int
            employeeId, int year, int month,
        CancellationToken cancellationToken = default)
    {
// TODO
        throw new NotImplementedException();
    ...
    /// <summary>
    ///     Calculates the total worked hours and overtime hours for the specified attendance record based on the clock-in and
    ///     clock-out times.
    ///     If a break duration is provided, it is subtracted from the total worked time.
    /// </summary>
    /// <param name="attendance">
    ///     The attendance record containing the clock-in, clock-out, and break duration information to
