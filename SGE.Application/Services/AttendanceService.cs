    ...
    IMapper mapper) : IAttendanceService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existingAttendance = await attendanceRepository.GetByEmployeeAndDateAsync(
            clockInDto.EmployeeId, date, cancellationToken);

        if (existingAttendance != null)
        {
            // Si une entrée existe et qu'elle a déjà un ClockIn
            if (existingAttendance.ClockIn.HasValue)
    ...
    ///     out, or when the employee has already clocked out on the same day.
    /// </exception>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Vérifier s'il existe déjà une entrée pour la date
        var attendance = await attendanceRepository.GetByEmployeeAndDateAsync(
            clockOutDto.EmployeeId, date, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existingAttendance = await attendanceRepository.GetByEmployeeAndDateAsync(
            createAttendanceDto.EmployeeId, createAttendanceDto.Date, cancellationToken);

        if (existingAttendance != null)
            throw new InvalidOperationException(
                $"Attendance record already exists for {createAttendanceDto.Date:yyyy-MM-dd}");
    ...
    public async Task<IEnumerable<AttendanceDto>> GetAttendancesByEmployeeAsync(int employeeId,
        ...
        CancellationToken cancellationToken = default)
    {
        var attendances = await attendanceRepository.GetByEmployeeIdAsync(
    ...
    public async Task<IEnumerable<AttendanceDto>> GetAttendancesByDateAsync(DateTime date,
        CancellationToken cancellationToken = default)
    {
        var attendances = await attendanceRepository.GetByDateAsync(date, cancellationToken);
        return mapper.Map<IEnumerable<AttendanceDto>>(attendances);
    }
    ...
    /// <exception cref="InvalidOperationException">
    ///     Thrown when multiple attendance records for the employee are found for the current date.
    /// </exception>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var attendance = await attendanceRepository.GetByEmployeeAndDateAsync(
            employeeId, today, cancellationToken);
    ...
    ///     clock-out times. If a break duration is provided, it is subtracted from the total worked time.
    /// </summary>
