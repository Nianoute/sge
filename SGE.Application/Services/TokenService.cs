...
public class TokenService(
    ...
    JwtSecurityTokenHandler tokenHandler) : ITokenService
{
    /// <summary>
    ///     Generates a JSON Web Token (JWT) access token for a specifieduser and their associated roles.
    /// </summary>
    ...
    public string GenerateAccessToken(ApplicationUser user, IList<string>
        roles)
    {
...
// Ajout des rôles
    ...
    ///     This allows for extracting user claims from a token that is no longer valid for authentication but still contains
    ///     valid claims data.
    /// </summary>
    ...
    ///     Thrown if the token is invalid or the algorithm used in the header is not
    ///     HmacSha256.
    /// </exception>
