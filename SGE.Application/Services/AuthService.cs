...
) : IAuthService
{
    ...
    public async Task<AuthResponseDto> RegisterAsync(RegisterDto
        registerDto)
    {
        var existingUserByEmail = await
            userManager.FindByEmailAsync(registerDto.Email);
        if (existingUserByEmail != null)
            throw new UserAlreadyExistsException(registerDto.Email);
        var existingUserByUsername = await
            userManager.FindByNameAsync(registerDto.UserName);
        ...
        var refreshToken = await
            tokenService.CreateRefreshTokenAsync(user.Id);
    ...
    /// <exception cref="UserNotActiveException">Thrown when the user account is disabled or inactive.</exception>
    public async Task<AuthResponseDto> RefreshTokenAsync(RefreshTokenDto
        refreshTokenDto)
    {
        var principal =
            tokenService.GetPrincipalFromExpiredToken(refreshTokenDto.AccessToken);
        var userId =
            principal.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (string.IsNullOrEmpty(userId))
            throw new InvalidRefreshTokenException();
        var isValidRefreshToken = await
            tokenService.ValidateRefreshTokenAsync(refreshTokenDto.RefreshToken,
                userId);
        ...
        if (user is not { IsActive: true })
            throw new UserNotActiveException();
        await
            tokenService.RevokeRefreshTokenAsync(refreshTokenDto.RefreshToken,
                "Remplacé par un nouveau token");
        var roles = await userManager.GetRolesAsync(user);
