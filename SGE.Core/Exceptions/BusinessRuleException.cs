...
///     The HTTP status code associated with this exception is 404 (NotFound).
/// </remarks>
...
{
    public LeaveRequestNotFoundException(int leaveRequestId) : base($"Demande de congé avec l'ID {leaveRequestId
    } introuvable.", "LEAVE_REQUEST_NOT_FOUND", 404)
    {
    }
}
...
/// </summary>
/// <remarks>
///     This exception is used to indicate that the number of leave days requested exceeds the number of leave days
...
{
    public ConflictingLeaveRequestException(DateTime startDate, DateTime
        endDate) :
...
{
    public InvalidLeaveStatusTransitionException(string currentStatus,
        string newStatus)
        : base($"Transition de statut invalide de '{currentStatus}' vers'{newStatus}'", "INVALID_STATUS_TRANSITION",
            400)
    {
    }
}
...
public class AlreadyClockedInException : SgeException
{
    public AlreadyClockedInException(int employeeId)
        : base($"L'employé {employeeId} est déjà pointé.",
...
public class NotClockedInException : SgeException
{
    public NotClockedInException(int employeeId)
        : base($"L'employé {employeeId} n'a pas pointé à l'arrivée.",
            "NOT_CLOCKED_IN", 400)
    {
    }
}
...
/// </summary>
/// <remarks>
///     This exception is specifically used to indicate issues where employee - related data is incorrect or does not
...
public class UserRegistrationException : SgeException
{
    public UserRegistrationException(IEnumerable<string> errors)
        : base($"Erreur lors de la création de l'utilisateur: {string.Join("; ", errors)}", "USER_REGISTRATION_FAILED",
            400)
    {
    }
}
